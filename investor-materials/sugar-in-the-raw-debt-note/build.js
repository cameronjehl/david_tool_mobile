#!/usr/bin/env node
/**
 * Builds the investor one-pager for each deal variant below:
 *   dist/<slug>/index.html       standalone web page (hover readout + schedule table)
 *   dist/<slug>/schedule.csv     the payment schedule as data
 *   canvas/<slug>/Main.dc.html   the same sheet as a Letter-size design artboard
 * Every number on a page is derived from the constants and its variant's term.
 *
 *   node build.js
 */
const fs = require('fs');
...
// One document per term. Principal is repaid as a balloon at the end of the term.
const VARIANTS = [
  { slug: '24-month', termMonths: 24 },
  { slug: '12-month', termMonths: 12 },
];
// ---------------------------------------------------------------------------
...
const pageTpl = read('templates/page.html');
const artboardTpl = read('templates/artboard.dc.html');
const canvasJSON = read('templates/canvas.json');

function buildVariant({ slug, termMonths: TERM_MONTHS }) {
  const dist = path.join(here, 'dist', slug);
  const canvasDir = path.join(here, 'canvas', slug);
  ...
  if (sumPayments !== totalInterest) throw new Error(`${slug}: payments ${sumPayments} != total interest ${totalInterest}`);
  ...
  const yStep = totalInterest > 30000 ? 10000 : 5000;
  const tickStep = TERM_MONTHS > 12 ? 6 : 3;
  ...
  function chartSVG({ interactive }) {
    ...
    for (let m = 0; m <= TERM_MONTHS; m += tickStep) {
    ...
    const desc = `Step chart rising from $0 at funding to ${usd(last.interestToDate)} at month ${TERM_MONTHS}, one step per monthly payment of ${usd(monthly, true)}. Principal of ${usd(PRINCIPAL)} returned at month ${TERM_MONTHS}.`;
...
${p.join('\n')}
  ...
  const artboard = fill(artboardTpl, { FONTS_CSS: fontsCSS, SHEET: artboardSheet });
  fs.writeFileSync(path.join(canvasDir, 'Main.dc.html'), artboard);
  fs.writeFileSync(path.join(canvasDir, 'canvas.json'), canvasJSON);
  ...
  console.log(`${slug}: ${usd(PRINCIPAL)} · ${pct(ANNUAL_RATE)}/yr · ${TERM_MONTHS} mo · monthly ${usd(monthly, true)} (last ${usd(lastPayment, true)}) · interest ${usd(totalInterest)} (${pct(totalInterest / PRINCIPAL)}) · total back ${usd(totalBack)}`);
}

VARIANTS.forEach(buildVariant);
fs.writeFileSync(path.join(here, 'dist', 'variants.json'), JSON.stringify(VARIANTS.map(v => v.slug)) + '\n');
console.log(`wrote dist/<slug>/index.html + schedule.csv and canvas/<slug>/Main.dc.html for: ${VARIANTS.map(v => v.slug).join(', ')}`);
